...
- **Internal component unintentionally exported** (accidental-export hygiene): an internal/debug activity/service/receiver/provider with `android:exported="true"`, or pre-31 default-export via an `<intent-filter>` with no explicit `exported`, that was never meant to be reachable. Distinct from the cases above — here the *export itself* is the bug, even if the handler isn't obviously sensitive. → **MASVS-PLATFORM-1** · MASWE-0066 (Insecure Intents) · **MASTG-TEST-0373** *Internal Component Unintentionally Exported*.
...
- **Loading content from untrusted / attacker-influenceable sources** — the most common WebView bug: a deep-link/Intent extra reaching `loadUrl`/`loadDataWithBaseURL`, and/or no `WebViewClient.shouldOverrideUrlLoading` allow-list (any URL the page navigates to just loads). → **MASVS-PLATFORM-2** · MASWE-0070 (JavaScript Loaded from Untrusted Sources) / MASWE-0071 (WebViews Loading Content from Untrusted Sources) · **MASTG-TEST-0027** *Testing for URL Loading in WebViews* (navigation override) + **MASTG-TEST-0032** (protocol handlers).
- **Safe Browsing disabled**: `WebSettings.setSafeBrowsingEnabled(false)` or the manifest meta-data `android.webkit.WebView.EnableSafeBrowsing` set to `false` → turns off Google Safe Browsing malware/phishing protection for content rendered in the WebView. → **MASVS-PLATFORM-2** · MASWE-0071 · **MASTG-TEST-0027** (checks "EnableSafeBrowsing Disabled" alongside the navigation-override check).
