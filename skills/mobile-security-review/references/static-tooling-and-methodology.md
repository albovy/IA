...
- Storage: unauthenticated object listing/read on the `*.appspot.com` / `firebasestorage` bucket.
  → **MASVS-STORAGE-2**; the live probe sits in **MASTG-TEST-0001** (Testing Local Storage for Sensitive Data — legacy umbrella; the Firebase `.json` check lives in its narrative) — verified the probe is documented there. Static discovery is per the techniques above; **no clean atomic test for open-backend rules exists → cite control + descriptive name "Firebase/cloud open-rules exposure (ID to confirm)"**, never invent one.
