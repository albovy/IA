...
- **NETWORK:** re-derive from the framework's own stack — **Flutter ignores NSC/ATS+proxy (Dart only)**; RN = OkHttp + JS fetch + JS/native pinning module; Cordova = WebView + `onReceivedSslError`; Xamarin = `ServerCertificateValidationCallback`. → MASVS-NETWORK-1 (MASWE-0050/-0052), NETWORK-2 (MASWE-0047). Cleartext permitted via a framework's **own** config (not the Android manifest/NSC or iOS ATS) has a dedicated atomic test — **`MASTG-TEST-0237`** (Cross-Platform Framework Configurations Allowing Cleartext Traffic): e.g. Capacitor `server.cleartext`/`androidScheme`, Cordova `config.xml`, RN/Flutter HTTP config — cite it for cross-platform cleartext rather than a generic MASWE-0050 alone.
